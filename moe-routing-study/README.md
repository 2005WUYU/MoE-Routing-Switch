...
512 步结果见[公开分析摘要](reports/h20_20260906/README.md)：存在性与首层尺度得到支持，核心工程损害假设尚未得到支持。当前使用 `python -m moe_study.engineering`，从现有检查点接着执行一次更新，比较“路由增损是否抵消一步连续收益”和“真实方向是否比同范数随机扰动更危险”。运行、文件提取、统计与后续 Loss Spike / μP 因果目标见[三步工程实验链](docs/engineering_chain.md)。旧 `causal` 入口保留为上一版诊断，不是本次主入口。
...
| `engineering.py`、`engineering_measure.py`、`engineering_statistics.py` | 收益抵消尺度、同起点方向比较、文档/执行/方向交叉区间与分辨率 |
